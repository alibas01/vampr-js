class Vampire {
  ...
  closestCommonAncestor(vampire) {
    let createrArray = [vampire];
    let createrThis = [this];
    let currentVampire = vampire.creator;
      for (let i = 0; i < vampire.numberOfVampiresFromOriginal; i++) {
        createrArray.push(currentVampire);
        currentVampire = currentVampire.creator;
      }
      currentVampire = this.creator;
      for (let i = 0; i < this.numberOfVampiresFromOriginal; i++) {
        createrThis.push(currentVampire);
        currentVampire = currentVampire.creator;
      }
      for (let creater of createrArray) {
        for (let create of createrThis) {
          if(creater === create) {
            return create;
          }
        }
